...
function run(){
    ...
    var templatePath =new File($.fileName).parent+"/TemplateWEBPACK/";
    try{
...
function eachDocument(path,adoc){
    ...
    for(var i=0,doc=app.activeDocument.layers;i<app.activeDocument.layers.length;i++){
        if(doc[i].visible){
            if(doc[i].typename == "LayerSet"){
                var resultHtml = replaceSymbol(doc[i],path,startMark,adoc);
    ...
    if(_html){
            var htmlfile = new File(path + "/public/index.html");
            var htmlread = "";
            htmlfile.open("r");
...
//生成 html内容 & 图片
function replaceSymbol(set,path,startMark,adoc){
    var modeTime_tack = 0;
    ...
    for(var j=set.layers.length-1;j>=0;j--){
          if(set.layers[j].visible){
                ...
                if(j==set.layers.length-1){//背景判断
                    ...
                    var savepath = new File(path+'/public/images/bg'+startMark+'.jpg');
                    ...
                    html_bg = html_bg.replace("{{BgImgSrc}}",'./images/'+arr[arr.length-1]).replace("{{modeTIme}}",set.layers[j].name);
                }else{
                    ...
                    var savepath = new File(path+'/public/images/'+startMark+'-'+(j+1)+'.png');
                    var exportOptions = new ExportOptionsSaveForWeb();
