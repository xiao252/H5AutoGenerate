...
function run(){
	...
	var whstr = prompt("输入设计稿宽高(键入格式：750,1334),默认'750*1334'", "1", "提示");
    var width = whstr.length>0?parseInt(whstr.split(',')[0]):750;
    var height = whstr.length>0?parseInt(whstr.split(',')[1]):1334;//1334
    var resolution = 72;
    var resampleMethod = ResampleMethod.AUTOMATIC;
    var amount = 50;
    adoc.resizeImage(width, height, resolution, resampleMethod, amount);
    var templatePath =new File($.fileName).parent+"/TemplateBG/";
    try{
...
function eachDocument(path,adoc){
    ...
    if(_html){
			...
			htmlread = htmlread.replace("{{resourcesTpl}}",resources.join(','));
            htmlfile.open("w");
            htmlfile.encoding = 'utf-8';
...
//生成 html内容 & 图片
function replaceSymbol(set,path,startMark,adoc,resources){
    var modeTime_tack = 0;
    var html_page = '\n\t<div class="page">{{imgItem}}\n\t</div>\n';
    var html_bg='\n\t\t<div class="full" style="background:url({{BgImgSrc}}) center / cover no-repeat;-webkit-animation: scaletest 1s ease-out {{modeTIme}}s 1 both"></div>';
    var html_img_t = '\n\t\t<div style="background:url({{imgSrc}}) center / cover no-repeat;position:absolute;{{imgCss}}-webkit-animation: {{aniName}} {{aniTime}}s ease {{modeTIme}}s 1 both;"></div>';
    var html_img = "";
    ...
    for(var j=set.layers.length-1;j>=0;j--){
          if(set.layers[j].visible){
                ...
                if(j==set.layers.length-1){//背景判断
					...
					resources.push("'/images/bg"+startMark+".jpg'");
                    ...
                    html_bg = html_bg.replace("{{BgImgSrc}}",'./images/'+arr[arr.length-1]).replace("{{modeTIme}}",isNaN(parseFloat(set.layers[j].name))?'1':set.layers[j].name);
                }else{
                    ...
                    var exportOptions = new ExportOptionsSaveForWeb();
					resources.push("'/images/"+startMark+"-"+(j+1)+".png'");
					exportOptions.format = SaveDocumentType.PNG;
                    exportOptions.PNG8 = false;
                    app.activeDocument.exportDocument(savepath,ExportType.SAVEFORWEB, exportOptions);
                    var arr = savepath.toString().split('/');
					var nameParams = set.layers[j].name.split('-');//解析图层名字符
					var html_img_t_result = html_img_t.replace("{{imgSrc}}",'./images/'+arr[arr.length-1]);
					html_img_t_result = html_img_t_result.replace("{{imgCss}}","z-index:"+(set.layers.length-j)+";left:"+left+"%;top:"+top+"%;width:"+width+"%;height:"+height+"%;")
					if(nameParams.length==1 && !isNaN(parseFloat(nameParams[0]))){//1参 情况
						html_img_t_result=html_img_t_result.replace("{{aniName}}",'fadeInUp');
						html_img_t_result=html_img_t_result.replace("{{aniTime}}",'1');
						html_img_t_result=html_img_t_result.replace("{{modeTIme}}",nameParams[0]);
					}else{
						html_img_t_result=html_img_t_result.replace("{{aniName}}",'fadeInUp');
						html_img_t_result=html_img_t_result.replace("{{aniTime}}",'1');
						html_img_t_result=html_img_t_result.replace("{{modeTIme}}",-(j-(set.layers.length-1))/2);
					}
					if(nameParams.length==3){//3参 情况
						html_img_t_result=html_img_t_result.replace("{{aniName}}",nameParams[0]);
						html_img_t_result=html_img_t_result.replace("{{aniTime}}",nameParams[1]);
						html_img_t_result=html_img_t_result.replace("{{modeTIme}}",nameParams[2]);
					}
					html_img += html_img_t_result;
                 }
